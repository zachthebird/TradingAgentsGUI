...
// Map old SVG fragment refs to comic-cast PNGs.
// agree/disagree/surprised collapse to 'reacting'; done/verdict-reaction → reacting.
const STATE_FRAMES = {
...
// Load SVG defs for speech bubbles, reaction chips, and mini-portraits.
// (Main character portraits now use comic-cast PNGs via STATE_FRAMES, not SVG <use> refs.)
...
function buildScene(){
  ...
  SEATS.forEach(seat=>{
    ...
    el.style.left = x + 'px';
    el.style.top = y + 'px';
    el.innerHTML = '<div class="rt-portrait comic-panel"><img src="'+getAssetURL(STATE_FRAMES[seat.id].idle)+'" alt="'+seat.name+'" loading="eager"></div>'
...
// ===== FRAME-SWAP ENGINE =====
// Normalise legacy reaction states (agree/disagree/surprised) to 'reacting' for PNG lookup.
function setSeatState(seatId, state){
  const el = seatEls[seatId];
  if (!el) return;
  const frames = STATE_FRAMES[seatId];
  if (!frames) return;
  // Collapse legacy reaction states into 'reacting'
  if (state === 'agree' || state === 'disagree' || state === 'surprised') state = 'reacting';
  if (!frames[state]) return;
  const img = el.querySelector('.rt-portrait img');
  if (!img) return;
  const targetSrc = getAssetURL(frames[state]);
  if (img.getAttribute('src') === targetSrc) return;
...
async function playFrom(idx){
  ...
  try {
    for (let i=idx; i<SCRIPT.length; i++){
      ...
      // Auto-scroll active speaker into view
      if (seatEls[line.id]) {
        seatEls[line.id].scrollIntoView({ behavior: prefersReducedMotion() ? 'auto' : 'smooth', block: 'center', inline: 'center' });
      }
...
function installLiveWiring(){
  ...
  window.__sseListeners.push((type, ev)=>{
    ...
    if (/think|start|begin|working|preparing|building|initializing/i.test(status)){
      ...
      // Auto-scroll active speaker into view
      if (seatEl) {
        seatEl.scrollIntoView({ behavior: prefersReducedMotion() ? 'auto' : 'smooth', block: 'center', inline: 'center' });
      }

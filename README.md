...
|---|---|---|
| PC-98 terminal UI | `web-ui/frontend/the-bazaar/terminal-live.js` + `terminal.css` | Default live view: SSE-driven report pages, typewriter + talk-bob animation, candlestick chart, attract mode, archive reader, stream-drop recovery |
| ADV/battle UI | `web-ui/frontend/the-bazaar/castle-council.js` + `castle.css` | Game Boy shell, battle view, verdict banner — loads only in `?ui=adv` mode |
...
| Character art | `web-ui/frontend/the-bazaar/design/pc98-cast/` (terminal) · `design/comic-cast/` (ADV) | 21 PC-98 finance-firm portraits · 8 painterly personas × 3 frames |
| Design refs | `web-ui/frontend/the-bazaar/design/` | UI spec, PC-98 reference image, full asset sheets |
